...
**`transactions`**: `TransactionGet[]` - Array of get operations generated with [`EntityManager.transactionGet(primaryKey, options?)`](/docs/guide/table/entityManager#entitytransactiongetprimarykey-options) method.
...
| Method                                                                                                                                                        | Description                                     |
| ------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------- |
| [`EntityManager.transactionPut(item, options?)`](/docs/guide/table/entityManager#entitytransactionputitem-options)                                            | Insert an item to DynamoDB.                     |
| [`EntityManager.transactionCreate(item, options?)`](/docs/guide/table/entityManager#entitytransactioncreateitem-options)                                      | Insert an item to DynamoDB (without overwrite). |
| [`EntityManager.transactionUpdate(primaryKey, props, options?)`](/docs/guide/table/entityManager#entitytransactionupdateprimarykey-props-options)             | Update an item in DynamoDB.                     |
| [`EntityManager.transactionDelete(primaryKey, options?)`](/docs/guide/table/entityManager#entitytransactiondeleteprimarykey-options)                          | Delete an item in DynamoDB.                     |
| [`EntityManager.transactionCondition(primaryKey, conditionInstance)`](/docs/guide/table/entityManager#entitytransactionconditionprimarykey-conditioninstance) | Condition for the transaction.                  |
...
const transactions = await transactionWrite(
  [
    ...
    KeyValueManager.transactionPut(
      new KeyValue({
        key: 'key2',
        value: { test: 'test2' },
      }),
    ),
    KeyValueManager.transactionCreate(
      new KeyValue({
        key: 'key3',
        value: { test: 'test3' },
      }),
    ),
    KeyValueManager.transactionDelete({ key: 'key4' }),
    KeyValueManager.transactionCondition({ key: 'key5' }, KeyValueManager.condition().attribute('key').eq('key5')),
  ],
);
// OR
const transactions = await transactionWrite(
