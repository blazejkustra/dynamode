...
## new Scan(Entity) or `Entity.scan()`

Every scan has to be initialized with [Entity](/docs/guide/entity/methods) to infer its underlying properties. You can achieve this in two ways:
...
```ts  
AllPossibleProperties.scan().attribute('LSI_1_SK').beginsWith('s').indexName('LSI_1_NAME'); // Resulting in: `IndexName: LSI_1_NAME`
```
...
```ts
User.scan().attribute('username').eq('blazej').totalSegments(10).segment(0); // Resulting in: `Segment: 0`
```
...
```ts
User.scan().attribute('username').eq('blazej').totalSegments(10).segment(0); // Resulting in: `TotalSegments: 10`
```
...
```ts
User.scan().attribute('username').eq('blazej').limit(10); // Resulting in: `Limit: 10`
```
...
```ts
const response1 = await User.scan().attribute('username').eq('blazej').run(); 
...
```ts
User.scan().attribute('username').eq('blazej').consistent(); // Resulting in: `ConsistentRead: true`
```
...
```ts
User.scan().attribute('username').eq('blazej').count(); // Resulting in: `Select: 'COUNT'`
```
...
```ts
User.scan().attribute('username').eq('blazej').attributes(['username', 'age']); // Resulting in: `ProjectionExpression: 'username, age'`
```
