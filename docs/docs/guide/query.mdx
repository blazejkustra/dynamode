...
## `new Query(Entity)` or `Entity.query()`

Every query has to be initialized with [Entity](/docs/guide/entity/methods) to infer its underlying properties. You can achieve this in two ways:

```ts
// Below definitions are equivalent
new Query(AllPossibleProperties);
AllPossibleProperties.query();
```
...
```ts
AllPossibleProperties.query().partitionKey('partitionKey');
AllPossibleProperties.query().partitionKey('GSI_1_PK');
```
:::
...
```ts
AllPossibleProperties.query().partitionKey('unknownProperty');
```
:::
...
```ts
User.query().partitionKey('partitionKey').eq('1'); // Resulting in: `partitionKey = 1`
```
...
```ts
AllPossibleProperties.query().sortKey('sortKey');
AllPossibleProperties.query().sortKey('GSI_1_SK');
AllPossibleProperties.query().sortKey('LSI_1_SK');
```

:::caution
Typescript error: Argument of type '"unknownProperty"' is not assignable to parameter of type: `"sortKey" | "GSI_1_SK" | "LSI_1_SK"`
```ts
AllPossibleProperties.query().sortKey('unknownProperty');
```
:::
...
```ts
User.query().sortKey('sortKey'); // This condition has no impact on the final conditional
User.query().sortKey('sortKey').eq('blazej'); // Adding comparison function (eq) after `sortKey(key)` method will complete the conditional. Resulting in: `sortKey = blazej`
```
...
```ts
User.query().sortKey('sortKey').eq('blazej'); // Resulting in: `sortKey = blazej`
```
...
```ts
User.query().sortKey('sortKey').ne('blazej'); // Resulting in: `sortKey <> blazej`
```
...
```ts
User.query().sortKey('sortKey').lt('blazej'); // Resulting in: `sortKey < blazej`
```
...
```ts
User.query().sortKey('sortKey').le('blazej'); // Resulting in: `sortKey <= blazej`
```
...
```ts
User.query().sortKey('sortKey').gt('blazej'); // Resulting in: `sortKey > blazej`
```
...
```ts
User.query().sortKey('sortKey').ge('blazej'); // Resulting in: `sortKey >= blazej`
```
...
```ts
User.query().sortKey('sortKey').beginsWith('bla'); // Resulting in: `begins_with(sortKey, bla)`
```
...
```ts
User.query().sortKey('sortKey').between('bla', 'cla'); // Resulting in: `sortKey BETWEEN 'bla' AND 'cla'`
```
...
```ts
User.query().partitionKey('partitionKey').eq('1').sort('descending'); // Resulting in: `ScanIndexForward: false`
```
...
```ts
User.query().partitionKey('partitionKey').eq('1').limit(10); // Resulting in: `Limit: 10`
```
...
```ts
const response1 = await User.query().partitionKey('partitionKey').eq('1').run(); 
const response2 = await User.query().partitionKey('partitionKey').eq('1').startAt(response1.lastKey).run(); // Resulting in: `LastEvaluatedKey: response1.lastKey`
```
...
```ts
User.query().partitionKey('partitionKey').eq('1').consistent(); // Resulting in: `ConsistentRead: true`
```
...
```ts
User.query().partitionKey('partitionKey').eq('1').count(); // Resulting in: `Select: 'COUNT'`
```
...
```ts
User.query().partitionKey('partitionKey').eq('1').attributes(['username', 'age']); // Resulting in: `ProjectionExpression: 'username, age'`
```
